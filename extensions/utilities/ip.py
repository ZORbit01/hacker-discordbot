...
@lightbulb.add_checks(lightbulb.has_roles(VERIFIED_ROLE_ID))
@lightbulb.command(
    "ip", "Lookup information of an IP Adress", aliases=["ipaddr"], auto_defer=True
)
...
async def ip_finder(ctx: lightbulb.Context, address) -> None:
    ...
    if ip_result:
        ...
        embd.add_field(
            name="Country Code (ISO):", value=country_code_iso3, inline=False
        )
        ...
        raise ValueError(
            "⚠ An Error Occured! Make sure the IP and the formatting are correct!"
        )
