...
NICKNAME_PERMISSIONS = Permissions.MANAGE_NICKNAMES | Permissions.MODERATE_MEMBERS
...
@lightbulb.option(
    "nick",
    "the new nickname you want to set (leave empty to reset!)",
    str,
    required=False,
)
@lightbulb.option(
    "member", "The member you want to change the nickname", hikari.Member, required=True
)
@lightbulb.add_checks(lightbulb.checks.has_role_permissions(NICKNAME_PERMISSIONS))
@lightbulb.app_command_permissions(NICKNAME_PERMISSIONS, dm_enabled=False)
@lightbulb.command(
    "nickname", "Change the nickname of a member", auto_defer=True, pass_options=True
)
